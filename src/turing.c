    ...
    while (reject == false && halt == false) {
        ...
        for (int i = 0; i < machine->instruction_set.count; i++) {
            if (strcmp(machine->instruction_set.instructions[i].state_input,
                ...
                for (int j = 0; j < machine->instruction_set.instructions[i].count;
                    j++) {
                    bool underbar_in_accept_str = strchr(machine->instruction_set.instructions[i]
                                                             .transition_command[j]
                                                             .accept_string,
                                                      '_')
                        != NULL;
                    bool is_observed_NULL               = machine->tape.data[machine->tape.pointer] == 0;
                    bool observed_char_in_accept_string = (strchr(machine->instruction_set.instructions[i]
                                                                      .transition_command[j]
                                                                      .accept_string,
                    ...
                    if (observed_char_in_accept_string || (is_observed_NULL && underbar_in_accept_str)) {
                        ...
                        // Change state
                        machine->current_state = ALLOC_STR(strlen(machine->instruction_set.instructions[i]
                                .transition_command[j]
                                .state_output));
                        machine->current_state = strcpy(
                            machine->current_state, machine->instruction_set.instructions[i].transition_command[j].state_output);
                        // Overwrite
                        if (machine->instruction_set.instructions[i]
                                .transition_command[j]
                                .write
                            != '/') {
                            machine->tape.data[machine->tape.pointer] = machine->instruction_set.instructions[i]
                                                                            .transition_command[j]
                                                                            .write;
                        }
                        // Move
                        ...
                        case 'R':
                            ...
                            if (machine->tape.pointer >= machine->tape.capacity) {
                                machine->tape.data = GROW_ARRAY(char, machine->tape.data,
